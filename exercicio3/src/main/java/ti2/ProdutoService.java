...
//	public void add(Request request, Response response) {
		String palavra = request.queryParams("palavra");
		int tamanho = Integer.parseInt(request.queryParams("tamanho"));
		String classificacao = request.queryParams("classificacao");

		Produto produto = new Produto(palavra, classificacao);

		produtoDAO.inserirPalavra(produto);
...
//		int id = Integer.parseInt(request.params(":id"));
		...
		Produto produto = (Produto) produtoDAO.getProduto(palavra);
		...
		if (produto != null) {
            ...
            return "<produto>\n" + 
            		"\t<palavra>" + produto.getPalavra() + "</palavra>\n" +
            		"\t<tamanho>" + produto.getTamanho() + "</tamanho>\n" +
            		"\t<classificacao>" + produto.getClassificacao() + "</classificacao>\n" +
            		"</produto>\n";
        } else {
            response.status(404); // 404 Not found
            return "Produto " + palavra + " não encontrado.";
        }
